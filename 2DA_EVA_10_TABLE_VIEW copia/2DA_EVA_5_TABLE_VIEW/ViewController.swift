...
//    } comand shift 7
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if segue.identifier == "imgSegue"{
            if let indexPath = tableViewR.indexPathForSelectedRow{
            let miDetalleViewController = segue.destinationViewController as! ViewControllerImagen
            
            miDetalleViewController.imgName = misImg[indexPath.row]
            miDetalleViewController.resName = misDatos[indexPath.row]
        }
        }
    }
